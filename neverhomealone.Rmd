---
title: "Never Home Alone Ecoregions"
...
library(viridisLite)
knitr::opts_chunk$set(comment="", cache=T, warning = F, message = F, fig.path = "images-nha/",
...
#iNaturalist observations
dat.all <- read_csv("data/observations-524273.csv.gz", guess_max=1e6) %>% 
...
common_names <- dat.common %>% count(species, common_name) %>% drop_na(species) %>% mutate(eithername = coalesce(common_name, species)) %>% select(species, eithername) %>% deframe()
```

```{r load_gbif}
world <- rnaturalearth::ne_countries(scale = "small", returnclass = "sf") %>% filter(continent!="Antarctica") %>% st_union()
ggplot() + 
  geom_sf(data=world, fill="black", color="black")+
  geom_sf(data=dat.points, aes(color=phylum), size=0.3) + theme_void() + 
  scale_color_brewer("",palette = "Set2", guide=guide_legend(override.aes = list(size=4))) +
  theme(plot.background = element_rect(fill="navyblue"), legend.text = element_text(color="white", face=2))
```
...
ggplot() +  geom_sf(data=world, fill="black", color="black")+
  geom_sf(data=hex.full, aes(fill=n_obs), linewidth=0.5, color="grey70") + 
