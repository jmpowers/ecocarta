...
phylum_pal <- set_names(brewer.pal(6, "Set1")[c(1,2,4,6,5,3)], levels(specs$phylum))
```
...
fviz_nbclust(decostand(dat.tri, "hellinger"), kmeans, method = "gap_stat")
```
...
topcounts <- dat.tri %>% as.data.frame() %>% rownames_to_column("grid_id") %>% mutate(grid_id=as.numeric(grid_id)) %>% 
  pivot_longer(-grid_id) %>% group_by(grid_id) %>% slice_max(value, n = 10, with_ties=F) %>% 
  summarize(top_species = paste(value, name, collapse = "\n"))

standout.summary <- standout %>% group_by(cluster) %>% 
  summarize(standouts = paste(sprintf("%.2f",scaled_mean), species, collapse = "\n"))
  
tri.full %>% left_join(topcounts) %>% left_join(standout.summary) %>% 
